    ...
    fetchDataFromApi(`/api/product/${context?.isOpenFullScreenPanel.id}`).then(
      (res) => {
        ...
        setFormFields({
          ...
          productRam:
            res?.data?.productRam?.length !== 0 ? res?.data?.productRam : [],
          size: res?.data?.size?.length !== 0 ? res?.data?.size : [],
          productWeight:
            res?.data?.productWeight !== 0 ? res?.data?.productWeight : [],
          bannerTitleName: res?.data?.bannerTitleName,
          bannerImages: res?.data?.bannerImages,
          isDisplayOnHomeBanner: res?.data?.isDisplayOnHomeBanner,
        });
        setProductCat(res?.data?.catId);
        setProductSubCat(res?.data?.subCatId);
        setProductThirdCat(res?.data?.thirdSubCatId);
        setProductIsFeatured(res?.data?.isFeatured);
        setProductRams(
          res?.data?.productRam?.length !== 0 ? res?.data?.productRam : []
        );
        setProductSize(res?.data?.size?.length !== 0 ? res?.data?.size : []);
        setProductWeight(
          res?.data?.productWeight !== 0 ? res?.data?.productWeight : []
        );
        setPreviews(res?.data?.images);
        setBannerPreviews(res?.data?.bannerImages);
        setCheckedSwitch(res?.data?.isDisplayOnHomeBanner);
