...
const HomeSliderBanners = () => {
  ...
  const deleteMultipleHS = () => {
    ...
    try {
      deleteMultipleData(`/api/homeSlider/deleteMultiple`, {
        id: sortedId,
      }).then((res) => {
        getData();
  ...
  const deleteData = (id) => {
    deleteDataFromApi(`/api/homeSlider/${id}`).then((res) => {
      if (!res?.error) {
        context?.openAlertBox("success", res?.message);
        getData();
      } else {
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 md:mt-2 justify-between p-5 mt-3 ">
